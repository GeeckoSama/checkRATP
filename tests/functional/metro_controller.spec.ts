...
test.group('MetroController - /metro/check', (group) => {
  ...
  test('should accept valid metro line number', async ({ client }) => {
    ...
    // La réponse doit être soit un message de trafic fluide, soit des incidents
    response.assertTextIncludes(['Trafic fluide', 'Ligne'])
  })
...
test.group('MetroController - /metro/lines', (group) => {
  ...
  test('should return lines with correct structure', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.isArray(lines)
    client.assert.isAtLeast(lines.length, 1)

    // Vérifier la structure d'une ligne
    const line = lines[0]
    client.assert.property(line, 'name')
    client.assert.property(line, 'id')
    client.assert.property(line, 'type')
  })

  test('should include metro lines', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.lengthOf(metroLines, 16)
  })

  test('should include RER lines', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.lengthOf(rerLines, 5)
  })

  test('should include line 1 with correct ID', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.isDefined(line1)
    client.assert.equal(line1.id, 'C01371')
    client.assert.equal(line1.type, 'metro')
  })

  test('should include line 8 with correct ID', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.isDefined(line8)
    client.assert.equal(line8.id, 'C01378')
    client.assert.equal(line8.type, 'metro')
  })

  test('should include line 12 with correct ID', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.isDefined(line12)
    client.assert.equal(line12.id, 'C01382')
    client.assert.equal(line12.type, 'metro')
  })

  test('should include RER A with correct ID', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.isDefined(rerA)
    client.assert.equal(rerA.id, 'C01742')
    client.assert.equal(rerA.type, 'rer')
  })

  test('should include special lines (3bis, 7bis)', async ({ client }) => {
    const response = await client.get('/metro/lines')
    ...
    client.assert.isDefined(line3bis)
    client.assert.equal(line3bis.id, 'C01386')

    client.assert.isDefined(line7bis)
    client.assert.equal(line7bis.id, 'C01387')
  })
